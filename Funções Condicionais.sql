...
DECLARE @varDescontoEconomy FLOAT, @varDescontoRegular FLOAT, @varDescontoDeluxe FLOAT

SET @varDescontoEconomy = 0.05
...
SELECT
	...
	UnitPrice,
	CASE
		WHEN ClassName = 'Economy' THEN (1 - @varDescontoEconomy) * UnitPrice
		WHEN ClassName = 'Regular' THEN (1 - @varDescontoRegular) * UnitPrice
		WHEN ClassName = 'Deluxe' THEN (1 - @varDescontoDeluxe) * UnitPrice
	END AS 'Desconto'
FROM
	DimProduct
...
SELECT
    ...
    ROUND(AVG(Weight) * 100, 2) AS 'Peso Total (kg)',
    CASE
...
SELECT
	...
	OpenDate,
	CloseDate,
	DATEDIFF(DAY, OpenDate, ISNULL(CloseDate, GETDATE())) AS 'Dias de Atividade'
FROM
	DimStore
ORDER BY
    DATEDIFF(DAY, OpenDate, ISNULL(CloseDate, GETDATE())) DESC
